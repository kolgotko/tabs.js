...
_createClass(Tabs,[{key:"add",value:function add(){var _this=this;var selector=arguments.length>0&&arguments[0]!==undefined?arguments[0]:".tab";var nodeList=document.querySelectorAll(selector);var nodes=[].slice.call(nodeList);nodes.forEach(function(node){var tab=new _tab2.default(node);_this._tabsPool.push(tab);node.addEventListener('click',function(){var behavior=tab.getBehavior();switch(behavior){case'toggle':_this.hideAll(tab);tab.toggle();break;default:_this.hideAll();tab.show();break;}});});}},{key:"hideAll",value:function hideAll(except){var tabs=this._tabsPool;tabs.forEach(function(tab){if(except===tab)return;tab.hide();});}}]);return Tabs;}();window.Tabs=Tabs;exports.default=Tabs;});
